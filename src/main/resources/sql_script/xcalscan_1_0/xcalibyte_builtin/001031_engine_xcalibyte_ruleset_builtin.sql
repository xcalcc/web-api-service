...
values
((select rule_set.id from xcalibyte."rule_set" left join xcalibyte."scan_engine" on xcalibyte.scan_engine.id = xcalibyte.rule_set.scan_engine_id where "scan_engine".name = 'Xcalibyte' and "rule_set".name = 'BUILTIN' and "rule_set".version ='1'), 'VUL', 'AOB', 'AOB', 'c,c++,java', null, '${rule.Xcalibyte.BUILTIN.1.AOB.name}', '1', '1', 'LIKELY', 'HIGH', '${rule.Xcalibyte.BUILTIN.1.AOB.detail}', '${rule.Xcalibyte.BUILTIN.1.AOB.description}', '${rule.Xcalibyte.BUILTIN.1.AOB.msg_template}', 'system', 'system')
ON CONFLICT DO NOTHING;
...
('en', 'rule.Xcalibyte.BUILTIN.1.AOB.detail', '### AOB'||chr(10)||'#### Abstract'||chr(10)||'The program is accessing data outside the declared boundary (before or after) of the intended buffer.'||chr(10)||''||chr(10)||'#### Explanation'||chr(10)||'Typically, this can allow attackers to cause a crash during program execution. A crash can occur when the code reads read sensitive information from other memory locations or causeble amount of data and assumes that a sentinel exists to stop the read operation, such as a NUL in a string. The expected sentinel might not be located in the out-of-bounds memory, causinfg excessive data to be read, leading to a segmentation fault or a buffer overflow. The software may modify an index or perform pointer arithmetic that references a memory location that is outside of the boundaries of the buffer. A subsequent read operation then produces undefined or unexpected results.'||chr(10)||''||chr(10)||'#### Example'||chr(10)||'````text'||chr(10)||'int assign(int* a, int i)'||chr(10)||'{'||chr(10)||'  return a[i];  /* called by main'||chr(10)||'                   a only has 2 elements but i is 2 */'||chr(10)||'}'||chr(10)||''||chr(10)||'int main() {'||chr(10)||'  int a[2] = {0, 1}, b;'||chr(10)||'  b = assign(a, 2);  // call assign with a and i'||chr(10)||'                     // a has two elements and i is 2'||chr(10)||'                     // a[2] is out-of-bound and also uninitialized'||chr(10)||'  printf("value of b = %d\n", b);'||chr(10)||'  return 0;'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.BUILTIN.1.AOB.detail', '### AOB'||chr(10)||'#### 概要'||chr(10)||'该程序正在预期缓存的已声明的界限外（界限之前或之后）访问数据。'||chr(10)||''||chr(10)||'#### 解释'||chr(10)||'通常情况下，这会允许攻击者在程序执行期间造成程序崩溃。当代码从其他内存位置读取敏感数据或可疑数据量并假定存在前哨来阻止读操作时（例如字符串里的空值），可能会发生崩溃。预期的前哨可能不位于越界内存里，致使读取过量数据，从而导致段错误或缓存溢出。软件可能修改引用在缓存界限之外的内存位置的索引或执行指针运算。随后的读运算便会产生未定义或意外结果。'||chr(10)||''||chr(10)||'#### 示例'||chr(10)||'````text'||chr(10)||'int assign(int* a, int i)'||chr(10)||'{'||chr(10)||'  return a[i];  /* called by main'||chr(10)||'                   a only has 2 elements but i is 2 */'||chr(10)||'}'||chr(10)||''||chr(10)||'int main() {'||chr(10)||'  int a[2] = {0, 1}, b;'||chr(10)||'  b = assign(a, 2);  // call assign with a and i'||chr(10)||'                     // a has two elements and i is 2'||chr(10)||'                     // a[2] is out-of-bound and also uninitialized'||chr(10)||'  printf("value of b = %d\n", b);'||chr(10)||'  return 0;'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
...
(rule_set_id, category, vulnerable, rule_code, language, url, name, severity, priority, likelihood, remediation_cost, detail, description, msg_template, created_by, modified_by)
values
((select rule_set.id from xcalibyte."rule_set" left join xcalibyte."scan_engine" on xcalibyte.scan_engine.id = xcalibyte.rule_set.scan_engine_id where "scan_engine".name = 'Xcalibyte' and "rule_set".name = 'BUILTIN' and "rule_set".version ='1'), 'VUL', 'DBF', 'DBF', 'c,c++', null, '${rule.Xcalibyte.BUILTIN.1.DBF.name}', '2', '2', 'LIKELY', 'MEDIUM', '${rule.Xcalibyte.BUILTIN.1.DBF.detail}', '${rule.Xcalibyte.BUILTIN.1.DBF.description}', '${rule.Xcalibyte.BUILTIN.1.DBF.msg_template}', 'system', 'system')
ON CONFLICT DO NOTHING;
...
('en', 'rule.Xcalibyte.BUILTIN.1.DBF.detail', '### DBF'||chr(10)||'#### Abstract'||chr(10)||'The program has freed some resources (e.g. heap memory, I/O stream object etc) multiple times.'||chr(10)||''||chr(10)||'#### Explanation'||chr(10)||'The program has called functions such as free(), close() etc multiple times to release the same resource object. This could cause inconsistent system such as corruption of the system''s heap management data strutures or I/O stream subclasses etc. This in turn may allow malicious users to access arbitrary memory or cause an IOException.'||chr(10)||''||chr(10)||''||chr(10)||'#### Example'||chr(10)||'````text'||chr(10)||'int func_1(void *p) {'||chr(10)||'  if (p != NULL) {'||chr(10)||'    free(p);   // free p'||chr(10)||'  }'||chr(10)||'}'||chr(10)||''||chr(10)||'int func_2(void *p) {'||chr(10)||'  if (p != NULL) {'||chr(10)||'    free(p);  // free p'||chr(10)||'  }'||chr(10)||'}'||chr(10)||''||chr(10)||'int main() {'||chr(10)||'  int i, *p, *q;'||chr(10)||'  p = malloc(10 * sizeof(int));'||chr(10)||'  if (p == NULL)'||chr(10)||'    return 1;'||chr(10)||'  for (i=0; i < 10; ++i)'||chr(10)||'    p[i] = i;'||chr(10)||'  q = p;'||chr(10)||'  func_1(p);  // free p the first time'||chr(10)||'  func_2(q);  // free p the second time due to value of p copied into q'||chr(10)||'  return 0;'||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.BUILTIN.1.DBF.detail', '### DBF'||chr(10)||'#### 概要'||chr(10)||'该程序已多次释放了某些资源（例如，堆内存、I/O流对象等）。'||chr(10)||''||chr(10)||'### 解释'||chr(10)||'该程序已多次调用了像free()、close()这样的函数来释放同一个资源对象。这会导致不一致的系统，例如系统的堆管理数据结构或I/O流子类等的损坏。这从而可能允许恶意用户访问任意内存或造成IOException。'||chr(10)||''||chr(10)||''||chr(10)||'#### 示例'||chr(10)||'````text'||chr(10)||'int func_1(void *p) {'||chr(10)||'  if (p != NULL) {'||chr(10)||'    free(p);   // free p'||chr(10)||'  }'||chr(10)||'}'||chr(10)||''||chr(10)||'int func_2(void *p) {'||chr(10)||'  if (p != NULL) {'||chr(10)||'    free(p);  // free p'||chr(10)||'  }'||chr(10)||'}'||chr(10)||''||chr(10)||'int main() {'||chr(10)||'  int i, *p, *q;'||chr(10)||'  p = malloc(10 * sizeof(int));'||chr(10)||'  if (p == NULL)'||chr(10)||'    return 1;'||chr(10)||'  for (i=0; i < 10; ++i)'||chr(10)||'    p[i] = i;'||chr(10)||'  q = p;'||chr(10)||'  func_1(p);  // free p the first time'||chr(10)||'  func_2(q);  // free p the second time due to value of p copied into q'||chr(10)||'  return 0;'||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
...
('en', 'rule.Xcalibyte.BUILTIN.1.DDV.detail', '### DDV'||chr(10)||'#### Abstract'||chr(10)||'Execution of this statement will be nullified by another statement following it, or the result of this statement is never used.'||chr(10)||''||chr(10)||'#### Explanation'||chr(10)||'Such error usually indicates either a typo or that some statement has been removed through time.'||chr(10)||''||chr(10)||'#### Example'||chr(10)||'````text'||chr(10)||'void assign(int input)'||chr(10)||'{'||chr(10)||'    int a = 0; // dead code'||chr(10)||'    a = input; // result of "a = 0" will be nullified by this statement'||chr(10)||'    printf("a value: %d", a);'||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.BUILTIN.1.DDV.detail', '### DDV'||chr(10)||'#### 概要'||chr(10)||'这个语句之后的另一个语句将使此语句的执行无效，或者从未使用过此语句的结果。'||chr(10)||''||chr(10)||'#### 解释'||chr(10)||'此类错误通常表明或是有输入错误，或者过了一段时间后删除了某些语句。'||chr(10)||''||chr(10)||'#### 示例'||chr(10)||'````text'||chr(10)||'void assign(int input)'||chr(10)||'{'||chr(10)||'    int a = 0; // dead code'||chr(10)||'    a = input; // result of "a = 0" will be nullified by this statement'||chr(10)||'    printf("a value: %d", a);'||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
...
(rule_set_id, category, vulnerable, rule_code, language, url, name, severity, priority, likelihood, remediation_cost, detail, description, msg_template, created_by, modified_by)
values
((select rule_set.id from xcalibyte."rule_set" left join xcalibyte."scan_engine" on xcalibyte.scan_engine.id = xcalibyte.rule_set.scan_engine_id where "scan_engine".name = 'Xcalibyte' and "rule_set".name = 'BUILTIN' and "rule_set".version ='1'), 'VUL', 'FAM', 'FAM', 'c,c++,java', null, '${rule.Xcalibyte.BUILTIN.1.FAM.name}', '1', '1', 'UNLIKELY', 'LOW', '${rule.Xcalibyte.BUILTIN.1.FAM.detail}', '${rule.Xcalibyte.BUILTIN.1.FAM.description}', '${rule.Xcalibyte.BUILTIN.1.FAM.msg_template}', 'system', 'system')
ON CONFLICT DO NOTHING;
...
(rule_set_id, category, vulnerable, rule_code, language, url, name, severity, priority, likelihood, remediation_cost, detail, description, msg_template, created_by, modified_by)
values
((select rule_set.id from xcalibyte."rule_set" left join xcalibyte."scan_engine" on xcalibyte.scan_engine.id = xcalibyte.rule_set.scan_engine_id where "scan_engine".name = 'Xcalibyte' and "rule_set".name = 'BUILTIN' and "rule_set".version ='1'), 'VUL', 'FMT', 'FMT', 'c,c++', null, '${rule.Xcalibyte.BUILTIN.1.FMT.name}', '1', '1', 'UNLIKELY', 'LOW', '${rule.Xcalibyte.BUILTIN.1.FMT.detail}', '${rule.Xcalibyte.BUILTIN.1.FMT.description}', '${rule.Xcalibyte.BUILTIN.1.FMT.msg_template}', 'system', 'system')
ON CONFLICT DO NOTHING;
...
(rule_set_id, category, vulnerable, rule_code, language, url, name, severity, priority, likelihood, remediation_cost, detail, description, msg_template, created_by, modified_by)
values
((select rule_set.id from xcalibyte."rule_set" left join xcalibyte."scan_engine" on xcalibyte.scan_engine.id = xcalibyte.rule_set.scan_engine_id where "scan_engine".name = 'Xcalibyte' and "rule_set".name = 'BUILTIN' and "rule_set".version ='1'), 'VUL', 'FMT', 'FMT-2', 'c,c++', null, '${rule.Xcalibyte.BUILTIN.1.FMT-2.name}', '1', '1', 'UNLIKELY', 'LOW', '${rule.Xcalibyte.BUILTIN.1.FMT-2.detail}', '${rule.Xcalibyte.BUILTIN.1.FMT-2.description}', '${rule.Xcalibyte.BUILTIN.1.FMT-2.msg_template}', 'system', 'system')
ON CONFLICT DO NOTHING;
...
('en', 'rule.Xcalibyte.BUILTIN.1.FMT-2.detail', '### FMT'||chr(10)||'#### Abstract'||chr(10)||'The program is calling one of printf family with number (or type) of paramter(s) used different from format string declaration.'||chr(10)||''||chr(10)||'#### Explanation'||chr(10)||'In a printf (or family of this kinds of system calls), the number or type of actual parameter used is different from that of the format string specifier. When the actual passed is different, what is actually printed will be unpredictable.'||chr(10)||''||chr(10)||'#### Example 1'||chr(10)||'````text'||chr(10)||''||chr(10)||'int foo(int a, int guard)'||chr(10)||'{'||chr(10)||'    int x = 0;'||chr(10)||'    int int_result = 0;'||chr(10)||''||chr(10)||'    if (guard != 0)'||chr(10)||'      int_result = a * x;'||chr(10)||''||chr(10)||'    // The format string specified for a "%c", but the parameter is of size int, result is truncated and unexpected'||chr(10)||'    printf("result %c is truncated\n", int_result);'||chr(10)||'    return 0;'||chr(10)||'}'||chr(10)||'````'||chr(10)||''||chr(10)||'#### Example 2'||chr(10)||'````text'||chr(10)||''||chr(10)||'int foo(int a, int guard)'||chr(10)||'{'||chr(10)||'    int x = 0;'||chr(10)||'    int int_result = 0;'||chr(10)||''||chr(10)||'    if (guard != 0)'||chr(10)||'      int_result = a * x;'||chr(10)||''||chr(10)||'    // The format string specified two values to be printed, only one actual parameter is passed. The second output is unpredictable'||chr(10)||'    printf("result %d, guard = %d\n", int_result);'||chr(10)||'    return 0;'||chr(10)||'}'||chr(10)||''||chr(10)||'`````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.BUILTIN.1.FMT-2.detail', '### FMT'||chr(10)||'#### 概要'||chr(10)||'该程序正调用printf函数族中的一个，其使用的参数个数（或类型）与格式字符串声明的不同。'||chr(10)||''||chr(10)||'#### 解释'||chr(10)||'在printf（或此类系统调用的家族）里，使用的实际参数的个数或类型与格式化字符串说明符的不同。当传递的实际参数不同时，则实际打印的内容将不可预测。'||chr(10)||''||chr(10)||'#### 示例 1'||chr(10)||'````text'||chr(10)||''||chr(10)||'int foo(int a, int guard)'||chr(10)||'{'||chr(10)||'    int x = 0;'||chr(10)||'    int int_result = 0;'||chr(10)||''||chr(10)||'    if (guard != 0)'||chr(10)||'      int_result = a * x;'||chr(10)||''||chr(10)||'    // The format string specified for a "%c", but the parameter is of size int, result is truncated and unexpected'||chr(10)||'    printf("result %c is truncated\n", int_result);'||chr(10)||'    return 0;'||chr(10)||'}'||chr(10)||'````'||chr(10)||''||chr(10)||'#### 示例 2'||chr(10)||'````text'||chr(10)||''||chr(10)||'int foo(int a, int guard)'||chr(10)||'{'||chr(10)||'    int x = 0;'||chr(10)||'    int int_result = 0;'||chr(10)||''||chr(10)||'    if (guard != 0)'||chr(10)||'      int_result = a * x;'||chr(10)||''||chr(10)||'    // The format string specified two values to be printed, only one actual parameter is passed. The second output is unpredictable'||chr(10)||'    printf("result %d, guard = %d\n", int_result);'||chr(10)||'    return 0;'||chr(10)||'}'||chr(10)||''||chr(10)||'`````', 'system', 'system'),
('en', 'rule.Xcalibyte.BUILTIN.1.FMT-2.msg_template', 'In ${se.filename}, line ${se.line}, the format statement in ${se.func}, argument ${se.num} has different type to that in the format string declaration.', 'system', 'system'),
...
(rule_set_id, category, vulnerable, rule_code, language, url, name, severity, priority, likelihood, remediation_cost, detail, description, msg_template, created_by, modified_by)
values
((select rule_set.id from xcalibyte."rule_set" left join xcalibyte."scan_engine" on xcalibyte.scan_engine.id = xcalibyte.rule_set.scan_engine_id where "scan_engine".name = 'Xcalibyte' and "rule_set".name = 'BUILTIN' and "rule_set".version ='1'), 'ROBUSTNESS', 'MSF', 'MSF', 'c,c++', null, '${rule.Xcalibyte.BUILTIN.1.MSF.name}', '1', '2', 'LIKELY', 'MEDIUM', '${rule.Xcalibyte.BUILTIN.1.MSF.detail}', '${rule.Xcalibyte.BUILTIN.1.MSF.description}', '${rule.Xcalibyte.BUILTIN.1.MSF.msg_template}', 'system', 'system')
ON CONFLICT DO NOTHING;
...
('en', 'rule.Xcalibyte.BUILTIN.1.NPD.detail', '### NPD (EXP34-C)'||chr(10)||'#### Abstract'||chr(10)||'The program is accessing memory through a pointer with NULL value.  This will cause a segmentation fault or unpredictable program behavior. This vulnerability is equivalent to EXP34-C'||chr(10)||''||chr(10)||'#### Explanation'||chr(10)||'On a system with memory protection, such as Linux, dereferencing a null pointer will cause a segmentation fault. For embedded systems, it will cause unpredicted program behavior. In Java, a null pointer dereference will trigger a null pointer exception.'||chr(10)||''||chr(10)||''||chr(10)||'#### Example - C'||chr(10)||'````text'||chr(10)||''||chr(10)||'extern int bar(int);'||chr(10)||''||chr(10)||'int assign(int* a)'||chr(10)||'{'||chr(10)||'  int i = bar(*a) // dereference a'||chr(10)||'  return i;'||chr(10)||'}'||chr(10)||''||chr(10)||'int foo(void)'||chr(10)||'{'||chr(10)||'  int *p = 0;  // p as a pointer has been initialized to 0 (null)'||chr(10)||'  assign(p);'||chr(10)||'}'||chr(10)||'````'||chr(10)||''||chr(10)||'#### Example - Java'||chr(10)||'````text'||chr(10)||'class User {'||chr(10)||'  public String getName() {'||chr(10)||'    return null;'||chr(10)||'  }'||chr(10)||'}'||chr(10)||''||chr(10)||'class Bar {'||chr(10)||'  private User findUser(String uid) {'||chr(10)||'    if (user.containsKey(uid)) {'||chr(10)||'      return user.get(uid);'||chr(10)||'    }'||chr(10)||'    else'||chr(10)||'      return null;'||chr(10)||'  }'||chr(10)||'  public void Npd(String uid) {'||chr(10)||'    // do something'||chr(10)||'    // ...'||chr(10)||'    User user = findUser(uid); // Throws NPE if "user" has not been properly initialized'||chr(10)||'    String getName() {'||chr(10)||'      return null;'||chr(10)||'    }'||chr(10)||'  }'||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.BUILTIN.1.NPD.detail', '### NPD (EXP34-C)'||chr(10)||'#### 概要'||chr(10)||'该程序正通过有空值的指针访问内存。这可能会导致段错误或不可预测的程序行为。该漏洞与EXP34-C等同'||chr(10)||''||chr(10)||'#### 解释'||chr(10)||'在有内存保护的系统里（例如Linux），解引用空指针会导致段错误。 对于嵌入系统来说，它会造成不可预测的程序行为。在Java里，空指针解引用会触发空指针异常。'||chr(10)||''||chr(10)||''||chr(10)||'#### 示例 - C'||chr(10)||'````text'||chr(10)||''||chr(10)||'extern int bar(int);'||chr(10)||''||chr(10)||'int assign(int* a)'||chr(10)||'{'||chr(10)||'  int i = bar(*a) // dereference a'||chr(10)||'  return i;'||chr(10)||'}'||chr(10)||''||chr(10)||'int foo(void)'||chr(10)||'{'||chr(10)||'  int *p = 0;  // p as a pointer has been initialized to 0 (null)'||chr(10)||'  assign(p);'||chr(10)||'}'||chr(10)||'````'||chr(10)||''||chr(10)||'#### 示例 - Java'||chr(10)||'````text'||chr(10)||'class User {'||chr(10)||'  public String getName() {'||chr(10)||'    return null;'||chr(10)||'  }'||chr(10)||'}'||chr(10)||''||chr(10)||'class Bar {'||chr(10)||'  private User findUser(String uid) {'||chr(10)||'    if (user.containsKey(uid)) {'||chr(10)||'      return user.get(uid);'||chr(10)||'    }'||chr(10)||'    else'||chr(10)||'      return null;'||chr(10)||'  }'||chr(10)||'  public void Npd(String uid) {'||chr(10)||'    // do something'||chr(10)||'    // ...'||chr(10)||'    User user = findUser(uid); // Throws NPE if "user" has not been properly initialized'||chr(10)||'    String getName() {'||chr(10)||'      return null;'||chr(10)||'    }'||chr(10)||'  }'||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
...
(rule_set_id, category, vulnerable, rule_code, language, url, name, severity, priority, likelihood, remediation_cost, detail, description, msg_template, created_by, modified_by)
values
((select rule_set.id from xcalibyte."rule_set" left join xcalibyte."scan_engine" on xcalibyte.scan_engine.id = xcalibyte.rule_set.scan_engine_id where "scan_engine".name = 'Xcalibyte' and "rule_set".name = 'BUILTIN' and "rule_set".version ='1'), 'VUL', 'RSX', 'RSX', 'c,c++', null, '${rule.Xcalibyte.BUILTIN.1.RSX.name}', '2', '2', 'LIKELY', 'LOW', '${rule.Xcalibyte.BUILTIN.1.RSX.detail}', '${rule.Xcalibyte.BUILTIN.1.RSX.description}', '${rule.Xcalibyte.BUILTIN.1.RSX.msg_template}', 'system', 'system')
ON CONFLICT DO NOTHING;

insert into xcalibyte.i18n_message
(locale, message_key, content, created_by, modified_by)
values
('en', 'rule.Xcalibyte.BUILTIN.1.RSX.description', 'The program has read from external sockets which may include untrusted data.', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.BUILTIN.1.RSX.description', '该程序已从可能包括不受信任数据的外部套接字中读取', 'system', 'system'),
('en', 'rule.Xcalibyte.BUILTIN.1.RSX.detail', '### RXS'||chr(10)||'#### Abstract'||chr(10)||'The program has read from external sockets which may include untrusted data.'||chr(10)||''||chr(10)||'#### Explanation'||chr(10)||'Because external sockets'||chr(10)||''||chr(10)||''||chr(10)||'#### Example'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <sys/socket.h>'||chr(10)||'#include <stdio.h>'||chr(10)||'#include <stdlib.h>'||chr(10)||'#include <unistd.h>'||chr(10)||''||chr(10)||'#define BUF_SZ 256'||chr(10)||''||chr(10)||'int foo(void)'||chr(10)||'{'||chr(10)||'  int n;'||chr(10)||'  char buffer[BUF_SZ];'||chr(10)||'  sockfd = socket(AF_INET, SOCK_STREAM, 0);'||chr(10)||''||chr(10)||'  if (sockfd < 0) {'||chr(10)||'    perror("ERROR opening socket");'||chr(10)||'    exit(1);'||chr(10)||'  }'||chr(10)||''||chr(10)||'  n = read(sockfd,buffer,255);'||chr(10)||''||chr(10)||'  // use buffer'||chr(10)||'  // if buffer is used as argument to system() or to setenv()'||chr(10)||'  // the buffer may contain untrusted commands or characters leading to unpredictable program behavior'||chr(10)||'  // ...'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.BUILTIN.1.RSX.detail', '### RXS'||chr(10)||'#### 概要'||chr(10)||'该程序已从可能包括不受信任数据的外部套接字中读取'||chr(10)||''||chr(10)||'#### 解释'||chr(10)||'Because external sockets'||chr(10)||''||chr(10)||''||chr(10)||'#### 示例'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <sys/socket.h>'||chr(10)||'#include <stdio.h>'||chr(10)||'#include <stdlib.h>'||chr(10)||'#include <unistd.h>'||chr(10)||''||chr(10)||'#define BUF_SZ 256'||chr(10)||''||chr(10)||'int foo(void)'||chr(10)||'{'||chr(10)||'  int n;'||chr(10)||'  char buffer[BUF_SZ];'||chr(10)||'  sockfd = socket(AF_INET, SOCK_STREAM, 0);'||chr(10)||''||chr(10)||'  if (sockfd < 0) {'||chr(10)||'    perror("ERROR opening socket");'||chr(10)||'    exit(1);'||chr(10)||'  }'||chr(10)||''||chr(10)||'  n = read(sockfd,buffer,255);'||chr(10)||''||chr(10)||'  // use buffer'||chr(10)||'  // if buffer is used as argument to system() or to setenv()'||chr(10)||'  // the buffer may contain untrusted commands or characters leading to unpredictable program behavior'||chr(10)||'  // ...'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
('en', 'rule.Xcalibyte.BUILTIN.1.RSX.msg_template', 'In ${se.filename}, line ${se.line}, the function ${se.func} is receiving untrusted data from external socket, the socket has been created in ${ss.filename} at line ${ss.line}.', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.BUILTIN.1.RSX.msg_template', 'In ${se.filename}, line ${se.line}, the function ${se.func} is receiving untrusted data from external socket, the socket has been created in ${ss.filename} at line ${ss.line}.', 'system', 'system'),
('en', 'rule.Xcalibyte.BUILTIN.1.RSX.name', 'Read From External Socket', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.BUILTIN.1.RSX.name', '从外部套接字读取', 'system', 'system')
ON CONFLICT (locale, message_key) DO UPDATE set content = excluded.content;

delete from xcalibyte.rule_information_attribute ria
where ria.rule_information_id =
      (select ri.id from xcalibyte."rule_information" ri left join xcalibyte."rule_set" rs on rs.id =ri.rule_set_id left join xcalibyte."scan_engine" se on se.id = rs.scan_engine_id
       where se.name = 'Xcalibyte' and rs.name = 'BUILTIN' and rs.version ='1' and ri.rule_code ='RSX');
...
('en', 'rule.Xcalibyte.BUILTIN.1.UAF.detail', '### UAF'||chr(10)||'#### Abstract'||chr(10)||'The program has referenced memory after it has been freed. It can cause the program to crash or unexpected program behavior.'||chr(10)||''||chr(10)||'#### Explanation'||chr(10)||'Use after free is a variation of dangling pointer reference. It typically occurs when the pointer was not updated after the memory object it points to has been freed. This pointer will be pointing to inappropriate memory leading to unauthorized access when the pointer is used.'||chr(10)||''||chr(10)||''||chr(10)||'#### Example'||chr(10)||'````text'||chr(10)||'int g = 2;'||chr(10)||''||chr(10)||'void my_free(void *p) {'||chr(10)||'  if (p != NULL)'||chr(10)||'    free(p);  // free p'||chr(10)||'}'||chr(10)||''||chr(10)||'int main() {'||chr(10)||'  int i, j, *p, *q;'||chr(10)||'  p = malloc(10 * sizeof(int));'||chr(10)||'  if (p == NULL)'||chr(10)||'    return 1;'||chr(10)||'  for (i=0; i < 10; ++i)'||chr(10)||'    p[i] = i;'||chr(10)||'  q = p;'||chr(10)||'  my_free(p);  //  p is freed'||chr(10)||'  j = 0;'||chr(10)||'  for (i=0; i < 10; ++i)'||chr(10)||'    j += q[i]; // Use after free here (note the statement q = p), read of q[i] is illegal'||chr(10)||'  return j;'||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.BUILTIN.1.UAF.detail', '### UAF'||chr(10)||'#### 概要'||chr(10)||'该程序在已释放了内存后引用了该内存。这可能导致程序崩溃或意外程序行为。'||chr(10)||''||chr(10)||'#### 解释'||chr(10)||'释放后重用是悬空指针引用的一种形式。它通常发生在指针所指向的内存对象释放后指针未更新时。该指针将指向不恰当的内存，这在使用指针时会导致未经授权的访问。'||chr(10)||''||chr(10)||''||chr(10)||'#### 示例'||chr(10)||'````text'||chr(10)||'int g = 2;'||chr(10)||''||chr(10)||'void my_free(void *p) {'||chr(10)||'  if (p != NULL)'||chr(10)||'    free(p);  // free p'||chr(10)||'}'||chr(10)||''||chr(10)||'int main() {'||chr(10)||'  int i, j, *p, *q;'||chr(10)||'  p = malloc(10 * sizeof(int));'||chr(10)||'  if (p == NULL)'||chr(10)||'    return 1;'||chr(10)||'  for (i=0; i < 10; ++i)'||chr(10)||'    p[i] = i;'||chr(10)||'  q = p;'||chr(10)||'  my_free(p);  //  p is freed'||chr(10)||'  j = 0;'||chr(10)||'  for (i=0; i < 10; ++i)'||chr(10)||'    j += q[i]; // Use after free here (note the statement q = p), read of q[i] is illegal'||chr(10)||'  return j;'||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
...
(rule_set_id, category, vulnerable, rule_code, language, url, name, severity, priority, likelihood, remediation_cost, detail, description, msg_template, created_by, modified_by)
values
((select rule_set.id from xcalibyte."rule_set" left join xcalibyte."scan_engine" on xcalibyte.scan_engine.id = xcalibyte.rule_set.scan_engine_id where "scan_engine".name = 'Xcalibyte' and "rule_set".name = 'BUILTIN' and "rule_set".version ='1'), 'VUL', 'UDR', 'UDR', 'c,c++', null, '${rule.Xcalibyte.BUILTIN.1.UDR.name}', '1', '1', 'UNLIKELY', 'MEDIUM', '${rule.Xcalibyte.BUILTIN.1.UDR.detail}', '${rule.Xcalibyte.BUILTIN.1.UDR.description}', '${rule.Xcalibyte.BUILTIN.1.UDR.msg_template}', 'system', 'system')
ON CONFLICT DO NOTHING;
...
(rule_set_id, category, vulnerable, rule_code, language, url, name, severity, priority, likelihood, remediation_cost, detail, description, msg_template, created_by, modified_by)
values
((select rule_set.id from xcalibyte."rule_set" left join xcalibyte."scan_engine" on xcalibyte.scan_engine.id = xcalibyte.rule_set.scan_engine_id where "scan_engine".name = 'Xcalibyte' and "rule_set".name = 'BUILTIN' and "rule_set".version ='1'), 'CORRECTNESS', 'UIV', 'UIV', 'c,java', null, '${rule.Xcalibyte.BUILTIN.1.UIV.name}', '2', '2', 'UNLIKELY', 'LOW', '${rule.Xcalibyte.BUILTIN.1.UIV.detail}', '${rule.Xcalibyte.BUILTIN.1.UIV.description}', '${rule.Xcalibyte.BUILTIN.1.UIV.msg_template}', 'system', 'system')
ON CONFLICT DO NOTHING;
