...
('en', 'rule.Xcalibyte.CERT.1.ARR38-C.detail', '### ARR38'||chr(10)||'#### Abstract'||chr(10)||'The program is calling a library function with a pointer and a size parameter. The two parameters, when combined, will be outside of legal range of the object.'||chr(10)||''||chr(10)||'#### Explanation'||chr(10)||'The program is passing a pointer to an object and a size parameter to the library function. The two parameters, when combined, will cause the library function to access this object but outside its valid range, resulting in undefined behavior.'||chr(10)||''||chr(10)||''||chr(10)||'#### Example'||chr(10)||'````text'||chr(10)||'#define ARRAY_SZ 2'||chr(10)||'int main()'||chr(10)||'{'||chr(10)||'  int a[ARRAY_SZ] = {0, 1}, dest[ARRAY_SZ];'||chr(10)||''||chr(10)||'  int *p_dest = &dest[1];  // p points to beginning of dest[1];'||chr(10)||'  memset(p_dest, a, sizeof(a)); // a + sizeof(a) will be one passed a[ARRAY_SZ]'||chr(10)||'  return 0;'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.ARR38-C.detail', '### ARR38'||chr(10)||'#### 概要'||chr(10)||'该程序正调用有指针和大小参数的库函数。当这两个参数组合在一起时，它们会在该对象的合法范围之外。'||chr(10)||''||chr(10)||'#### 解释'||chr(10)||'该程序正传递指向对象的指针和大小参数给库函数。当这两个参数组合在一起时，将导致库函数在该对象的有效范围之外访问它，从而造成未定义行为。'||chr(10)||''||chr(10)||''||chr(10)||'#### 示例'||chr(10)||'````text'||chr(10)||'#define ARRAY_SZ 2'||chr(10)||'int main()'||chr(10)||'{'||chr(10)||'  int a[ARRAY_SZ] = {0, 1}, dest[ARRAY_SZ];'||chr(10)||''||chr(10)||'  int *p_dest = &dest[1];  // p points to beginning of dest[1];'||chr(10)||'  memset(p_dest, a, sizeof(a)); // a + sizeof(a) will be one passed a[ARRAY_SZ]'||chr(10)||'  return 0;'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.ENV33-C.detail', '### ENV33'||chr(10)||'#### Abstract'||chr(10)||'The program is calling the C standard system() function. When not properly protected, it could lead to various exploitations.'||chr(10)||'#### Explanation'||chr(10)||'The program is calling the system() function where the parameter is a character string. Such a string must be properly protected such as sanitized string, normalized path etc. Failure to do so will cause arbitrary program execution, privilege elevation or other unpredictable behavior.'||chr(10)||''||chr(10)||'#### Example'||chr(10)||'````text'||chr(10)||''||chr(10)||'void func_call_sys(const char *in)'||chr(10)||'{'||chr(10)||'  // system() is called with a string "in" passed from outside of this function'||chr(10)||'  // There is no evidence that "in" has been sanitized'||chr(10)||'  // for example, if "in" is the string "rm *"'||chr(10)||'  // executing the system() call could be devastating'||chr(10)||'  system(in);'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.ENV33-C.detail', '### ENV33'||chr(10)||'#### 概要'||chr(10)||'该程序正调用C标准system()函数。如果保护不当，可能会导致多种攻击。'||chr(10)||'#### 解释'||chr(10)||'该程序正调用有字符串参数的system()函数。必须适当地保护此类字符串，例如消过毒的字符串、规范化的路径等等。未能这么做将导致任意程序执行、权限提升或其它不可预测行为。'||chr(10)||''||chr(10)||'#### 示例'||chr(10)||'````text'||chr(10)||''||chr(10)||'void func_call_sys(const char *in)'||chr(10)||'{'||chr(10)||'  // system() is called with a string "in" passed from outside of this function'||chr(10)||'  // There is no evidence that "in" has been sanitized'||chr(10)||'  // for example, if "in" is the string "rm *"'||chr(10)||'  // executing the system() call could be devastating'||chr(10)||'  system(in);'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.ENV33-C.name', 'Please pay attention to the direct call to system()', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.FIO30-C.detail', '### FIO30'||chr(10)||'#### Abstract'||chr(10)||'The program has a format specification that contains a parameter of char * type (aka string) with an untrusted source and is considered tainted.'||chr(10)||'#### Explanation'||chr(10)||'Formatted I/O functions can be tricked into overriding or reading the value of any arbitrary memory location. When the input source is untrusted, it is better not to use format strings and related functions to perform I/O operation.'||chr(10)||''||chr(10)||''||chr(10)||'#### Example 1'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <stdio.h>'||chr(10)||''||chr(10)||'int main(int argc, char *argv[])'||chr(10)||'{'||chr(10)||'  int var_to_hold_content;  // the variable where printf will store content of an address'||chr(10)||'  if (argc > 2) {'||chr(10)||'      scanf(argv[1]);  // get a string from stdin (user input)'||chr(10)||'      printf(argv[1]); // user can read any address to a variable using a format specification'||chr(10)||'  }'||chr(10)||'}'||chr(10)||''||chr(10)||'````'||chr(10)||'#### Example 2'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <stdio.h>'||chr(10)||''||chr(10)||'int write_to_any_addr(void)'||chr(10)||'{'||chr(10)||'  int i = 0xDEAD;'||chr(10)||'  printf("\x12\x34\x56\x78  %x %x %x %x %n", &i);  // content of memory 0x12345678 will be overwritten as 0xDEAD'||chr(10)||'                                                   // if the format specifier is to be input from stdin as in example 1,'||chr(10)||'                                                   // user can read any memory in this process'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO30-C.detail', '### FIO30'||chr(10)||'#### 概要'||chr(10)||'该程序有格式规范，它包含有不受信任来源的char *类型（也即字符串）的参数，被看作是受了污染。'||chr(10)||'#### 解释'||chr(10)||'能诱导格式化的I/O函数覆盖或读取任何任意内存位置的值。当输入来源不受信任时，最好不要使用格式字符串以及相关函数来执行I/O操作。'||chr(10)||''||chr(10)||''||chr(10)||'#### 示例 1'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <stdio.h>'||chr(10)||''||chr(10)||'int main(int argc, char *argv[])'||chr(10)||'{'||chr(10)||'  int var_to_hold_content;  // the variable where printf will store content of an address'||chr(10)||'  if (argc > 2) {'||chr(10)||'      scanf(argv[1]);  // get a string from stdin (user input)'||chr(10)||'      printf(argv[1]); // user can read any address to a variable using a format specification'||chr(10)||'  }'||chr(10)||'}'||chr(10)||''||chr(10)||'````'||chr(10)||'#### 示例 2'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <stdio.h>'||chr(10)||''||chr(10)||'int write_to_any_addr(void)'||chr(10)||'{'||chr(10)||'  int i = 0xDEAD;'||chr(10)||'  printf("\x12\x34\x56\x78  %x %x %x %x %n", &i);  // content of memory 0x12345678 will be overwritten as 0xDEAD'||chr(10)||'                                                   // if the format specifier is to be input from stdin as in example 1,'||chr(10)||'                                                   // user can read any memory in this process'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.FIO30-C.name', 'Format specification should not have parameters that are strings from an untrusted source', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.FIO34-C.detail', '### FIO34'||chr(10)||'#### Abstract'||chr(10)||'The program is using return value of file read (such as getchar(), getc(), getwc(),..) to check for EOF/WEOF as end of file read operation.'||chr(10)||'#### Explanation'||chr(10)||'The functions that reads characters from a file return an int type. Directly comparing the character read with EOF/WEOF may cause an unexpected result because EOF and WEOF are implementation defined.'||chr(10)||''||chr(10)||'#### Example'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <stdio.h>'||chr(10)||''||chr(10)||'void read_char(void)'||chr(10)||'{'||chr(10)||'  FILE *fp = fopen("/tmp/myfile", "+r");'||chr(10)||'  // check fp valid'||chr(10)||'  ...'||chr(10)||''||chr(10)||'  int c = getc(fp);'||chr(10)||'  while (c != EOF)  // this check does not guarantee read has reached end of file'||chr(10)||'  {'||chr(10)||'    putchar(c);   // echo what is read in'||chr(10)||'    c = getc(fp);'||chr(10)||'  }'||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO34-C.detail', '### FIO34'||chr(10)||'#### 概要'||chr(10)||'该程序正使用文件读（例如getchar()、getc()、getwc()......）的返回值来核实EOF/WEOF为文件读操作的末尾。'||chr(10)||'#### 解释'||chr(10)||'从文件中读取字符的函数会返回int类型。把读取的字符直接和EOF/WEOF比较可能导致意外结果，因为EOF和WEOF是以实现为准的。'||chr(10)||''||chr(10)||'#### 示例'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <stdio.h>'||chr(10)||''||chr(10)||'void read_char(void)'||chr(10)||'{'||chr(10)||'  FILE *fp = fopen("/tmp/myfile", "+r");'||chr(10)||'  // check fp valid'||chr(10)||'  ...'||chr(10)||''||chr(10)||'  int c = getc(fp);'||chr(10)||'  while (c != EOF)  // this check does not guarantee read has reached end of file'||chr(10)||'  {'||chr(10)||'    putchar(c);   // echo what is read in'||chr(10)||'    c = getc(fp);'||chr(10)||'  }'||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.FIO34-C.name', 'EOF, WEOF as "char" size objects are different from chars read in from a file', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.FIO37-C.detail', '### FIO37'||chr(10)||'#### Abstract'||chr(10)||'The program is calling fgets() or fgetws() and assuming the string returned an non-empty string.'||chr(10)||'#### Explanation'||chr(10)||'The two functions may return a null string (e.g. the file may be a binary file).'||chr(10)||''||chr(10)||'#### Example'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <stdio.h>'||chr(10)||''||chr(10)||'#define BUF_SZ  1024'||chr(10)||''||chr(10)||'void read_string(void)'||chr(10)||'{'||chr(10)||'  char buf[BUF_SZ];'||chr(10)||'  FILE *fp = fopen("/myfile", "+r");'||chr(10)||'  // check fp valid'||chr(10)||'  // ...'||chr(10)||''||chr(10)||'  if (fgets(buf, BUF_SZ, fp) != 0) {'||chr(10)||'    // if the first character of buf is ''\0'', a large number will be printed'||chr(10)||'    printf("Size of string read = %d", strlen(buf)-1);'||chr(10)||'  }'||chr(10)||''||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO37-C.detail', '### FIO37'||chr(10)||'#### 概要'||chr(10)||'该程序正调用fgets()或fgetws()，并假定返回的字符串为非空字符串。'||chr(10)||'#### 解释'||chr(10)||'fgets() 和 fgetws()这两个函数可能返回空字符串（例如，文件可能为二进制文件）。'||chr(10)||''||chr(10)||'#### 示例'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <stdio.h>'||chr(10)||''||chr(10)||'#define BUF_SZ  1024'||chr(10)||''||chr(10)||'void read_string(void)'||chr(10)||'{'||chr(10)||'  char buf[BUF_SZ];'||chr(10)||'  FILE *fp = fopen("/myfile", "+r");'||chr(10)||'  // check fp valid'||chr(10)||'  // ...'||chr(10)||''||chr(10)||'  if (fgets(buf, BUF_SZ, fp) != 0) {'||chr(10)||'    // if the first character of buf is ''\0'', a large number will be printed'||chr(10)||'    printf("Size of string read = %d", strlen(buf)-1);'||chr(10)||'  }'||chr(10)||''||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO37-C.msg_template', 'In ${se.filename}, ${se.func}, line ${se.line} has an illegal access due to empty string although the string function in ${ss.func}, line ${ss.line} returns successfully.', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO37-C.msg_template', '在${se.filename}，第${se.line}行的函数${se.func}，会因为空字符串而導至非法访问，需然第${ss.line}行的函数${ss.func}的返回是成功的。', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO37-C.name', 'fgets() and fgetws() may return an empty string', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.FIO42-C.detail', '### FIO42'||chr(10)||'#### Abstract'||chr(10)||'The program has opened a file but failed to close the file when done with it or before the program terminates.'||chr(10)||'#### Explanation'||chr(10)||'It is important to close a file when it is no longer needed. Failure to do so may expose resource (such as non-flushed data) associated with the file to attackers.'||chr(10)||''||chr(10)||'#### Example'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <stdio.h>'||chr(10)||''||chr(10)||'int open_file(const char *fname)'||chr(10)||'{'||chr(10)||'  FILE *fp = fopen(fname, "+r");'||chr(10)||'  if (fp == NULL) return -1;'||chr(10)||''||chr(10)||'  // do something'||chr(10)||'  return 0;       // file is not properly closed on return'||chr(10)||''||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO42-C.detail', '### FIO42'||chr(10)||'#### 概要'||chr(10)||'该程序打开了文件但未能在使用完文件后或程序终止前关闭该文件。'||chr(10)||'#### 解释'||chr(10)||'当不再需要文件时就关闭它是很重要的。未能这么做可能会把与文件相关的资源（例如非刷新数据）暴露给攻击者。'||chr(10)||''||chr(10)||'#### 示例'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <stdio.h>'||chr(10)||''||chr(10)||'int open_file(const char *fname)'||chr(10)||'{'||chr(10)||'  FILE *fp = fopen(fname, "+r");'||chr(10)||'  if (fp == NULL) return -1;'||chr(10)||''||chr(10)||'  // do something'||chr(10)||'  return 0;       // file is not properly closed on return'||chr(10)||''||chr(10)||'}'||chr(10)||'````', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO42-C.msg_template', 'In ${se.file} function ${se.func} line ${se.line}, we detected a ${rule.Xcalibyte.CERT.1.FIO42-C.msg_template.se.1}. ${rule.mT.sub.FIOC42C.s3.1} to ${s3.var} at line ${s3.line} in function ${s3.func}. ${rule.Xcalibyte.CERT.1.FIO42-C.msg_template.s4.1} at line ${s4.line} in function ${s4.func}. ${rule.Xcalibyte.CERT.1.FIO42-C.msg_template.se.2} ${se.var} at line ${se.line}, function ${se.func}.', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO42-C.msg_template', '在${se.file} 第${se.line}上的函数 ${se.func}，我们检测到了${rule.Xcalibyte.CERT.1.FIO42-C.msg_template.se.1}. 在函数${s3.func},第${s3.line}行, ${rule.mT.sub.FIOC42C.s3.1} 变量${s3.var}的. 第${s4.line}行的函数${s4.func} ${rule.Xcalibyte.CERT.1.FIO42-C.msg_template.s4.1}. 第${se.line}行, 函数${se.func} ${rule.Xcalibyte.CERT.1.FIO42-C.msg_template.se.2}里的变量${se.var}.', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO42-C.msg_template.s3.1', 'The file was opened with "r" attribute and assigned to file handle', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO42-C.msg_template.s3.1', '使用“w”属性打开了文件并分配给文件句柄', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO42-C.msg_template.s4.1', 'the file handle is checked and validated', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO42-C.msg_template.s4.1', '检查并验证了文件句柄', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO42-C.msg_template.se.1', 'file not closed defect', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO42-C.msg_template.se.1', '文件未关闭缺陷', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO42-C.msg_template.se.2', 'The function is terminated but the file not properly closed with file handle', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO42-C.msg_template.se.2', '终止了该函数，但未使用文件句柄适当地关闭文件', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO42-C.name', 'Close files when done with them', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.FIO45-C.detail', '### FIO45'||chr(10)||'#### Abstract'||chr(10)||'Race conditions while accessing files (Time of check, time of use) may happen and should b avoided'||chr(10)||'#### Explanation'||chr(10)||'In a shared file system, two or more processes may access the same file, causing race condition is possible. Attackers can change the file between two accesses by different processes. Or replace the file (symbolic or hard linked) to a different file.'||chr(10)||''||chr(10)||'#### Example'||chr(10)||'````text'||chr(10)||''||chr(10)||'FIX ME'||chr(10)||''||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO45-C.detail', '### FIO45'||chr(10)||'#### 概要'||chr(10)||'在访问文件（Time of check，time of use）时竞态条件可能发生，这应该要避免'||chr(10)||'#### 解释'||chr(10)||'在共享文件系统里，两个或多个进程可能访问同一个文件，从而导致竞态条件。攻击者可以在不同进程的两次访问之间更改文件，或用不同的文件代替该文件（符号或硬链接）。'||chr(10)||''||chr(10)||'#### 示例'||chr(10)||'````text'||chr(10)||''||chr(10)||'FIX ME'||chr(10)||''||chr(10)||'````', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO45-C.msg_template', 'In ${se.file} function ${se.func}, line ${se.line}, we detected a ${rule.Xcalibyte.CERT.1.FIO45-C.msg_template.se.1}. ${rule.Xcalibyte.CERT.1.FIO45-C.msg_template.s5.1}  ${s5.var} at line ${s5.line} in function ${s5.func}. ${rule.Xcalibyte.CERT.1.FIO45-C.msg_template.s4.1} at line ${s4.line}, function ${s4.func}. ${rule.Xcalibyte.CERT.1.FIO45-C.msg_template.se.2} ${se.var} at line ${se.line}, function ${se.func}.', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO45-C.msg_template', '在${se.file} 第${se.line}行的函数 ${se.func}，我们检测到了${rule.Xcalibyte.CERT.1.FIO45-C.msg_template.se.1}。第${s5.line}行的函数 ${s5.func} ${rule.Xcalibyte.CERT.1.FIO42-C.msg_template.s5.1}变量${s5.var}。'||chr(10)||'在函数${s4.func},第${s4.line}行 ${rule.Xcalibyte.CERT.1.FIO42-C.msg_template.s4.1}。${rule.Xcalibyte.CERT.1.FIO45-C.msg_template.se.2}在第${se.line}行的函数${se.func}里的变量${se.var}。', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO45-C.msg_template.s4.1', 'the file handle is checked and validated', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO45-C.msg_template.s4.1', '检查并验证了该文件', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO45-C.msg_template.s5.1', 'The file was opened for write and assigned to file handle', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO45-C.msg_template.s5.1', '使用"w"属性打开了该文件并分配给文件句柄', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO45-C.msg_template.se.1', 'file access TOCTOU (time of check, time of use) defect', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO45-C.msg_template.se.1', '文件访问TOCTOU （time of check， time of use）缺陷', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO45-C.msg_template.se.2', 'The TOCTOU is caused by the fwrite() to', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO45-C.msg_template.se.2', 'TOCTOU由fwrite()造成', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.FIO45-C.name', 'Files access may have race conditions and should be avoided', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.FIO47-C.detail', '<< FIX ME >>', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.FIO47-C.detail', '<< FIX ME >>', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.MEM30-C.detail', '### MEM30 (UAF)'||chr(10)||'#### Abstract'||chr(10)||'The program is accessing memory that has been freed. This vulnerability is the same as the Xcalibyte UAF rule.'||chr(10)||'#### Explanation'||chr(10)||'Please refer to rule UAF in Xcalibyte vulnerability list for detailed explanation and example.'||chr(10)||''||chr(10)||'#### Example'||chr(10)||''||chr(10)||'````text'||chr(10)||''||chr(10)||'struct S {'||chr(10)||'  int *p;'||chr(10)||'};'||chr(10)||''||chr(10)||'int main() {'||chr(10)||'  int *p;'||chr(10)||'  struct S *s;'||chr(10)||'  p = malloc(10 * sizeof(int));  // heap memory allocated and pointed to by p'||chr(10)||'  if (p == NULL)'||chr(10)||'    return 1;'||chr(10)||'  s = (struct S*)malloc(sizeof(struct S));  // heap memory allocated and pointed to by s'||chr(10)||'  if (s == NULL) {'||chr(10)||'    free(p);'||chr(10)||'    return 1;'||chr(10)||'  }'||chr(10)||'  s->p = p;'||chr(10)||'  free(s);    // only s is freed'||chr(10)||'              // s->p, which is copied from p is not freed'||chr(10)||'  return 0;'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.MEM30-C.detail', '### MEM30 (UAF)'||chr(10)||'#### 概要'||chr(10)||'该程序正访问已被释放的内存。该漏洞与鉴释UAF规则相同。'||chr(10)||'#### 解释'||chr(10)||'请参考鉴释漏洞列表里的UAF规则以获得详细的解释和示例。'||chr(10)||''||chr(10)||'#### 示例'||chr(10)||''||chr(10)||'````text'||chr(10)||''||chr(10)||'struct S {'||chr(10)||'  int *p;'||chr(10)||'};'||chr(10)||''||chr(10)||'int main() {'||chr(10)||'  int *p;'||chr(10)||'  struct S *s;'||chr(10)||'  p = malloc(10 * sizeof(int));  // heap memory allocated and pointed to by p'||chr(10)||'  if (p == NULL)'||chr(10)||'    return 1;'||chr(10)||'  s = (struct S*)malloc(sizeof(struct S));  // heap memory allocated and pointed to by s'||chr(10)||'  if (s == NULL) {'||chr(10)||'    free(p);'||chr(10)||'    return 1;'||chr(10)||'  }'||chr(10)||'  s->p = p;'||chr(10)||'  free(s);    // only s is freed'||chr(10)||'              // s->p, which is copied from p is not freed'||chr(10)||'  return 0;'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.MEM30-C.msg_template', 'In ${se.filename}, line ${se.line}, the variable ${se.var} in ${se.func} has not been freed. ${se.var} is first assigned heap memory acquired in ${ss.file} at line ${ss.line}.', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.MEM30-C.msg_template', '在${se.filename}，第${se.line}行，使用了 ${se.func} 里的变量${se.var}。然而，它在${ss.file} 第${ss.line}行被释放。', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.POS34-C.detail', '### POS34'||chr(10)||'#### Abstract'||chr(10)||'The program has called putenv() with an argument that points to a local object.'||chr(10)||'#### Explanation'||chr(10)||'The system call putenv() will save the argument (which is a pointer to the intended string) into the environment array. When the function returns to the caller, the local object with the environment string may be overwritten. This will cause unpredictable program behavior.'||chr(10)||''||chr(10)||'#### Example'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <string.h>'||chr(10)||''||chr(10)||'#define BUF_SZ  1024'||chr(10)||''||chr(10)||'int func(char *env_str)'||chr(10)||'{'||chr(10)||'  char buf[BUF_SZ];'||chr(10)||'  // ...'||chr(10)||'  if (strlen(env_str) < BUF_SZ) {'||chr(10)||'    strcpy(buf, env_str);'||chr(10)||'    return putenv(buf);'||chr(10)||'  }'||chr(10)||'  else {'||chr(10)||'    // handle error'||chr(10)||'    // ...'||chr(10)||'  }'||chr(10)||'  return 0;'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.POS34-C.detail', '### POS34'||chr(10)||'#### 概要'||chr(10)||'该程序调用了putenv()，它有指向本地对象的参数。'||chr(10)||'#### 解释'||chr(10)||'系统调用putenv()会保存其为指向预期字符串的指针的参数进环境数组。当该函数返回到调用者时，可能会重写有环境字符串的本地对象。这会造成不可预测的程序行为。'||chr(10)||''||chr(10)||'#### 示例'||chr(10)||'````text'||chr(10)||''||chr(10)||'#include <string.h>'||chr(10)||''||chr(10)||'#define BUF_SZ  1024'||chr(10)||''||chr(10)||'int func(char *env_str)'||chr(10)||'{'||chr(10)||'  char buf[BUF_SZ];'||chr(10)||'  // ...'||chr(10)||'  if (strlen(env_str) < BUF_SZ) {'||chr(10)||'    strcpy(buf, env_str);'||chr(10)||'    return putenv(buf);'||chr(10)||'  }'||chr(10)||'  else {'||chr(10)||'    // handle error'||chr(10)||'    // ...'||chr(10)||'  }'||chr(10)||'  return 0;'||chr(10)||'}'||chr(10)||''||chr(10)||'````', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.POS34-C.msg_template', 'In ${se.filename} at line ${se.line} the function ${se.func} is calling the system function putenv() with ${se.var} as parameter. This variable is a local variable declared at ${ss.line} in ${ss.func}.', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.POS34-C.msg_template', '在${se.filename}，第${se.line}行的函数 ${se.func} 正用变量${se.var}作为参数调用系统函数putenv()。该变量是在第${ss.line}行 ${ss.func}函数里的声明為本地变量。', 'system', 'system'),
...
('en', 'rule.Xcalibyte.CERT.1.POS37-C.detail', '### POS37'||chr(10)||'#### Abstract'||chr(10)||'The program has one of the get/set uid/euid set of functions which manipulates user id privileges. The result of calling such functions is not checked which will cause the program''s privilege to be in an unexpected state.'||chr(10)||'#### Explanation'||chr(10)||'Privilege capabilities are implementation defined. To ensure that the privileges are set and relinquished as expected, it is important to check the error conditions on return. Failure to do this may result in the program being run with root permission.'||chr(10)||''||chr(10)||'#### Example (proper usage)'||chr(10)||'````text'||chr(10)||''||chr(10)||'  // ...'||chr(10)||''||chr(10)||'  // the following check alone is not sufficient. need to make sure privilege can be restored'||chr(10)||'  if (setuid(getuid()) != 0) {'||chr(10)||'    // Handle error'||chr(10)||'  }'||chr(10)||''||chr(10)||'  // check for failure to setuid when the caller is UID 0'||chr(10)||'  // failure to do this will result in serious security risk'||chr(10)||'  if (setuid(0) != -1) {'||chr(10)||'    // privilege can be restored. Handle error hee'||chr(10)||'  }'||chr(10)||'````', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.POS37-C.detail', '### POS37'||chr(10)||'#### 概要'||chr(10)||'该程序有一组get/set uid/euid函数，它对用户id权限进行操作。调用此类函数的结果不受检查，这将导致该程序的权限处于意外状态。'||chr(10)||'#### 解释'||chr(10)||'权限能力是以实现为准的。要确保按预期设置及撤回权限，检查返回时错误条件是很重要的。未能这么做可能导致用根权限运行该程序。'||chr(10)||''||chr(10)||'#### 示例（正确用法）'||chr(10)||'````text'||chr(10)||''||chr(10)||'  // ...'||chr(10)||''||chr(10)||'  // the following check alone is not sufficient. need to make sure privilege can be restored'||chr(10)||'  if (setuid(getuid()) != 0) {'||chr(10)||'    // Handle error'||chr(10)||'  }'||chr(10)||''||chr(10)||'  // check for failure to setuid when the caller is UID 0'||chr(10)||'  // failure to do this will result in serious security risk'||chr(10)||'  if (setuid(0) != -1) {'||chr(10)||'    // privilege can be restored. Handle error hee'||chr(10)||'  }'||chr(10)||'````', 'system', 'system'),
('en', 'rule.Xcalibyte.CERT.1.POS37-C.msg_template', 'In ${se.filename}, function ${se.func} at line ${se.line}, privilege has not be properly restored. The privilege was reliquished in ${ss.func} at line ${ss.line}.', 'system', 'system'),
('zh-CN', 'rule.Xcalibyte.CERT.1.POS37-C.msg_template', '在${se.filename}，第${se.line}行的函数 ${se.func}，尚未适当地恢复权限。该权限在第${ss.line}行的函数${ss.func}被撤回。', 'system', 'system'),
